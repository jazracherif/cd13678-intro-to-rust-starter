...
mod tests {
    ...
    const WINDOW: game_ffi::Window = game_ffi::Window {
        width: 800,
        height: 600,
        sprite_side: 60,
    };
    ...
    fn test_sprite_flicker() {    
        ...
        START_WINDOW_AND_GAME_LOOP!(LOOP_SLEEP_MS, {
            red = match red {
                true => {
                    CHANGE_SPRITE_COLOR!(sprite, 0, 255, 0);
                    false
                }
                false => {
                    CHANGE_SPRITE_COLOR!(sprite, 255, 0, 0);
                    true
                }
            };
        });
        ...
        START_WINDOW_AND_GAME_LOOP!(LOOP_SLEEP_MS, {
            if now.elapsed() >= switch_sprite_in_ms {
                unsafe {
                    ...
                    red = match red {
                        true => {
                            game_ffi::render_sprite(sprite_red);
                            false
                        }
                        false => {
                            game_ffi::render_sprite(sprite_green);
                            true
                        }
                    };
                }
